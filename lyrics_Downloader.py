...
def scrape():
    for url in URLs:
        ...
        albums = {}
        album_artist = {}
        for key, value in small_artists:
...
def download_lyrics():
    ...
    rappers = []
    for url in URLs:   # Alle rapper finden
    ...
    for artist in rappers:
        if 'html' in artist['href']:
            ...
            soup = get_html(BASE_URL + artist['href'])
            tables = soup.find_all("table")[2:]
            for t in tables:
                links = t.find_all("a", href=True)[1:]
                stored_obj = {}
                ...
                albums_big_artists.append(stored_obj)
